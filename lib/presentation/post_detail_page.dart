...
class _PostDetailPageState extends State<PostDetailPage> {
          ...
          slivers: [
            SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                    background: _post!.images == null || _post!.images!.isEmpty
                        ? SizedBox(
                            child: Center(
  ...
  Widget _applicationSection() {
    ...
    if (!isLogged) return _beforeApplicationSection();
    ...
    // 신청 내역이 없는 경우
    if (app.id == null) return _beforeApplicationSection();
    ...
    switch (app.status) {
      ...
      default:
        return _beforeApplicationSection();
    }
  }
  ...
  Widget _beforeApplicationSection() {
    return BlocBuilder<AppBloc, AppState>(builder: (context, state) {
      if (state.state == UserAppState.requestCompletedState) {
        context.read<DataBloc>().add(ReloadPostEvent());
        context.read<AppBloc>().add(RequestCompletedEvent());
