...
class _PostMainPageState extends State<PostMainPage> {
  ...
  void initState() {
    ...
    _page++;
    widget.type == 'recruit'
        ? context.read<RecruitPostBloc>().add(RequestRecruitmentPaginationEvent(_page, 20))
        : context.read<PromotionBloc>().add(RequestPromotionPaginationEvent(_page, 20));
  }
          ...
          children: [
            Expanded(
              child: CustomRefreshIndicator(
                onRefresh: () async {
                  final recruit = context.read<RecruitPostBloc>();
                  final promotion = context.read<PromotionBloc>();

                  if (recruit.state.state == RecruitPostLoadState.dataLoadState
                      || promotion.state.state == PromotionPostLoadState.postLoadingState) {
                    return;
                  }
                  if(_isLast == true){
                    return;
                  ...
                  widget.type == 'recruit'
                      ? context.read<RecruitPostBloc>().add(RequestRecruitmentPaginationEvent(_page+1, 20))
                      : context.read<PromotionBloc>().add(RequestPromotionPaginationEvent(_page+1, 20));
                  widget.type == 'recruit'
                      ? await recruit.stream.firstWhere((state) => state is RecruitPaginationState)
                      : await promotion.stream.firstWhere((state) => state is PromotionPaginationState);
                ...
                builder: (context, child, controller) {
                  return Stack(
                    children: [
                      child,
                      if (controller.isDragging || controller.isAnimating)
                        Positioned(
                          bottom: 20,
                          left: 0,
                          right: 0,
  ...
  Widget _recruitSelector(context, double hei) {
    return BlocListener<RecruitPostBloc, RecruitPostState>(
        listener: (context, state) {
          ...
          if (state is RecruitPaginationState) {
            provider.getPostPagination(state.resPost, state.page);
            setState(() {
              _page = state.page;
        ...
        child: Selector<RecruitPostProvider, List<RecruitPostEntity>>(
            ...
            builder: (context, posts, child) {
              return _postListBuilder(context, posts, hei);
            }));
  }
  ...
  Widget _promotionSelector(context, double hei) {
    return BlocListener<PromotionBloc, PromotionPostState>(
        listener: (context, state) {
          ...
          if (state is PromotionPaginationState) {
            provider.getPagination(state.resPosts, state.page);
            setState(() {
              _page = state.page;
              _isLast = state.isLast;
            });
  ...
  Widget _postListBuilder(BuildContext context, List<dynamic> posts, double hei) {
    return Padding(
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          Text(
                                              isRecruit
                                                  ? (posts[idx] as RecruitPostEntity).platform!.name.toUpperCase()
                                      ...
                                      Text(
                                        ...
                                        style: TextStyle(
                                            color: Colors.grey.shade600, fontSize: 15, fontWeight: FontWeight.w500),
                                      )
                                    ],
                                  ),
                                )),
